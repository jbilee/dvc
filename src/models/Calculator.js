...
class Calculator {
  ...
  addListeners() {
    STAT_LISTS.start.forEach((stat) => $(stat).addEventListener("change", () => this.checkDullAvailability()));
    $("#dragon-selector").addEventListener("change", (e) => this.selectDragon(e));
    $("#start-trait-selector").addEventListener("change", (e) => this.selectStartTrait(e));
    $("#normal-trait-selector").addEventListener("change", (e) => this.selectNormalTrait(e));
    $("#special-trait-selector").addEventListener("change", (e) => this.selectSpecialTrait(e));
    $("#btn-calculate").addEventListener("click", () => this.calculate());
    $("#btn-reset").addEventListener("click", () => this.reset());
    $("#poisoned").addEventListener("change", (e) => this.handleCheckPoisoned(e));
  }
  ...
  handleCheckPoisoned(e) {
    ...
    if (!this.poisonedValue && maxStatNames.length > 1)
      displayToast(
        this.language === "ko" ? "맹독은 한 가지 노력치에만 적용할 수 있습니다." : "Venom applies to only one stat.",
        3700
      );
    ...
    if (e.target.checked) {
      $(`#start-${maxStatName}`).value = maxStatValue - 10 <= 0 ? 0 : maxStatValue - 10;
  ...
  checkDullAvailability() {
    ...
    const isAvailable = currentStartValues.getMaxStatValue() <= 25 ? true : false;
  ...
  selectDragon(e) {
    ...
    const { traitsEn, traitsKo } = dragonList.find(({ name }) => name[0] === dragonName);
    ...
    secondTrait.textContent = this.language === "ko" ? traitsKo[1] : traitsEn[1];
  ...
  selectSpecialTrait(e) {
    ...
    switch (traitSelected) {
      case "Refined": {
        ...
        if (goal[highestStatName] >= 50) {
          return this.printStatFields(goal, "#end-");
        }

        goal[highestStatName] = 50;

        goal[highestStatName] = this.getOptimizedValue(goal[highestStatName] - base[highestStatName]) + base[highestStatName];

        if (this.highestFirst) {
          goal[highestStatName] = this.replaceWithNine(goal[highestStatName] - base[highestStatName]) + base[highestStatName];
        }
      ...
      case "Mischievous": {
        ...
        while (goal.hasSameStats()) {
          ...
          const duplicateValues = STAT_LISTS.base.filter(
            (stat) => allValues.indexOf(goal[stat]) !== allValues.lastIndexOf(goal[stat])
          );
      ...
      case "Distracted": {
        ...
        if (curFocus === 30) {
          ...
          const optimizedMaxGoal = this.getOptimizedValue(goal[maxGoalStat] - base[maxGoalStat]) + base[maxGoalStat];
          const finalMaxGoal = this.replaceWithNine(optimizedMaxGoal - base[maxGoalStat]) + base[maxGoalStat];
        ...
        if (curFocus === 10 && goal.getMaxStatValue() === 30) {
          ...
          if (allValues.indexOf(10) !== allValues.lastIndexOf(10)) {
            ...
            goal[zeroKey] = 27;
            const leftoverKey = STAT_LISTS.base.filter((stat) => goal[stat] === 10 && stat !== "focus");
            goal[leftoverKey] = 42;
            ...
            const [zero1, zero2] = STAT_LISTS.base.filter((stat) => goal[stat] === 0 && stat !== "focus");
            goal[zero1] = 57;
            goal[zero2] = 27;
        ...
        if (curFocus === 10 && goal.getMaxStatValue() === 20) {
          if (baseStatValues.indexOf(0) !== baseStatValues.lastIndexOf(0)) {
            const [zero1, zero2] = STAT_LISTS.base.filter((stat) => base[stat] === 0);
            goal[zero1] = 45;
            goal[zero2] = 63;
            const maxBaseStat = base.getMaxStatName();
            goal[maxBaseStat] = 25;
            return this.printStatFields(goal, "#end-");
          } else if (baseStatValues.indexOf(0) >= 0 && baseStatValues.indexOf(0) === baseStatValues.lastIndexOf(0)) {
            const [zeroKey] = STAT_LISTS.base.filter((stat) => base[stat] === 0);
            goal[zeroKey] = 45;
            const [maxBaseStat] = base.getMaxStatName();
            goal[maxBaseStat] = 25;
            const [remainingKey] = STAT_LISTS.base.filter(
              (stat) => stat !== "focus" && stat !== zeroKey && stat !== maxBaseStat
            );
        ...
        if (
          ...
          const [zeroKey] = STAT_LISTS.base.filter((stat) => base[stat] === 0);
          goal[zeroKey] = 36;
          const [remainingFive] = STAT_LISTS.base.filter((stat) => base[stat] === 5 && stat !== "focus");
        ...
        if (curFocus < 15) {
          ...
          const remainingStats = STAT_LISTS.base.filter((x) => !excludedStats.includes(x) && x !== "focus");
          ...
          if (remainingStats.length === 2) {
            const changesToMake = this.compareTrainingCounts(base, remainingStats, keptValues);
            changesToMake.forEach((change) => {
              goal[change.statName] = change.value;
            });
            const maxGoalStat = goal.getMaxStatName();
        ...
        if (curFocus > 15 && curFocus < 30) {
          ...
          if (remainingStats.length === 2 && base[remainingStats[0]] === base[remainingStats[1]]) {
            ...
            switch (targetValue) {
              case 0: {
                if (curFocus === 20) {
                  ...
                  const remainingValues = this.highestFirst ? [54, 36] : [50, 36];
                  if (this.preference !== "none" && this.preference !== "focus") {
                    goal[this.preference] = remainingValues.shift();
                    const lastStat = remainingStats.filter((stat) => stat !== this.preference);
                ...
                if (curFocus === 25) {
                  ...
                  const remainingValues = this.highestFirst ? [63, 41] : [57, 41];
                  if (this.preference !== "none" && this.preference !== "focus") {
                    goal[this.preference] = remainingValues.shift();
                    const lastStat = remainingStats.filter((stat) => stat !== this.preference);
                    goal[lastStat] = remainingValues[0];
              ...
              case 5: {
                if (curFocus === 20) {
                  ...
                  if (remainingStats.includes(this.preference)) {
                    ...
                    const prefStat = remainingStats.includes("strength") ? "strength" : "intellect";
                    goal[prefStat] = remainingValues.shift();
                    const lastStat = remainingStats.filter((stat) => stat !== prefStat);
                    goal[lastStat] = remainingValues[0];
                  }
                }
                if (curFocus === 25) {
            ...
            const changesToMake = this.compareTrainingCounts(base, remainingStats, goals);
            changesToMake.forEach((change) => {
              goal[change.statName] = change.value;
            });
            const highestGoal = goal.getMaxStatName();
      ...
      case "Dull": {
        try {
          ...
          // Optimization
          if (maxValue < 25) {
            let countsArray = STAT_LISTS.base.map((stat) => getTargetSum(goal[stat] - base[stat], TRAIN_VALUES));

            while (countsArray.includes(null)) {
              STAT_LISTS.base.forEach((stat) => (goal[stat] += 1));
              countsArray = STAT_LISTS.base.map((stat) => getTargetSum(goal[stat] - base[stat], TRAIN_VALUES));
            }
            ...
            const tempArray = STAT_LISTS.base.map((stat) => getTargetSum(tempGoal[stat] - base[stat], TRAIN_VALUES));
            const tempTrainingCounts = tempArray.reduce((sum, cur) => {
              if (cur.length === 0) return sum;
              return sum + cur.length;
            }, 0);

            if (tempGoal.getMaxStatValue() < 25 && tempTrainingCounts < currentTrainingCounts)
      ...
      case "Iridescent": {
        if (base.getMinStatValue() === 1 && base.getMaxStatValue() === 1) {
          const goal = new Stats(25, 25, 25, 25);
          if (this.preference !== "none") {
            goal[this.preference] = this.highestFirst ? 55 : 35;
          } else {
            goal.strength = this.highestFirst ? 55 : 35;
          }
          return this.printStatFields(goal, "#end-");
        }
      ...
      case "Capable": {
        ...
        const containsEight = STAT_LISTS.base.map((stat) => {
          const trainingCounts = getTargetSum(goal[stat] - base[stat], TRAIN_VALUES);
          return trainingCounts.includes(5) && trainingCounts.includes(3) ? true : false;
        });
        ...
        const containsSix = STAT_LISTS.base.map((stat) => {
          const trainingCounts = getTargetSum(goal[stat] - base[stat], TRAIN_VALUES);
          return trainingCounts.indexOf(3) !== trainingCounts.lastIndexOf(3) ? true : false;
        });
        const containsSixSet = new Set(containsSix);
        if (containsSixSet.size === 1 && containsSixSet.has(true) && goal.getMaxStatValue() >= 27) {
          STAT_LISTS.base.forEach((stat) => {
            goal[stat] -= 1;
          });
        }
      ...
      default: {
        ...
        STAT_LISTS.base.forEach((stat) => (goal[stat] = this.getOptimizedValue(goal[stat] - base[stat]) + base[stat]));

        if (this.highestFirst) {
          STAT_LISTS.base.forEach((stat) => (goal[stat] = this.replaceWithNine(goal[stat] - base[stat]) + base[stat]));
        }
  ...
  selectNormalTrait(e) {
    ...
    if (startingTrait === traitSelected) return this.printStatFields(base, "#end-");
    ...
    if (lowestReqStat === "none") req = this.doubleTraining(base, highestReqStat);
  ...
  doubleTraining(base, highestReqStat) {
    ...
    if (!highestStats.includes(highestReqStat) || (highestStats.includes(highestReqStat) && highestStats.length > 1))
      goal[highestReqStat] = highestValue + 1;

    while (EXCLUDED_VALUES.includes(goal[highestReqStat] - base[highestReqStat])) {
      goal[highestReqStat] += 1;
    }

    // Optimization
    goal[highestReqStat] = this.getOptimizedValue(goal[highestReqStat] - base[highestReqStat]) + base[highestReqStat];

    if (this.highestFirst) {
      goal[highestReqStat] = this.replaceWithNine(goal[highestReqStat] - base[highestReqStat]) + base[highestReqStat];
    }
  ...
  singleTraining(base, highestReqStat, lowestReqStat) {
    const goal = this.copyStats(base);
    const targetStats = Object.keys(goal).filter((key) => key !== highestReqStat && key !== lowestReqStat);
    ...
    if (!goal.getMaxStatName().includes(highestReqStat) || goal.getMaxStatName().length > 1) {
  ...
  getOptimizedValue(value) {
    ...
    if (trainingCounts.includes(5) && trainingCounts.includes(3)) trainingCounts = this.optimizeByOne(trainingCounts);
  ...
  replaceWithNine(value) {
    ...
    while (trainingCounts.includes(5) || trainingCounts.includes(3)) {
      if (trainingCounts.includes(5)) trainingCounts.splice(trainingCounts.indexOf(5), 1, 9);
      if (trainingCounts.includes(3)) trainingCounts.splice(trainingCounts.indexOf(3), 1, 9);
    }
    return getArraySum(trainingCounts);
  }
